        ...
        for (int i = 0; i < board.length; i++) {
            for (int j = 0; j < board[i].length; j++) {
                if (board[i][j] instanceof ClearCell) {
                    if (i - 1 >= 0) {
                        if (j - 1 >= 0) ((ClearCell) board[i][j]).addNeighbour(board[i - 1][j - 1]);
                        ((ClearCell) board[i][j]).addNeighbour(board[i - 1][j]);
                        if (j + 1 < board[0].length) ((ClearCell) board[i][j]).addNeighbour(board[i - 1][j + 1]);
                    }
                    if (j - 1 >= 0) ((ClearCell) board[i][j]).addNeighbour(board[i][j - 1]);
                    if (j + 1 < board[0].length) ((ClearCell) board[i][j]).addNeighbour(board[i][j + 1]);
